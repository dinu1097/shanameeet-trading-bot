...
// Simulate one random stock trade
...
// Display simulated trade chains
echo "<h2 class='text-lg font-semibold mb-2'>Simulated Trade Chains</h2>";
echo "<table class='min-w-full table-auto border mb-6'>";
echo "<thead>
...
// Load and show user simulated trades from data.json
$dataFile = 'data.json';
$userTrades = file_exists($dataFile) ? json_decode(file_get_contents($dataFile), true) : [];

echo "<h2 class='text-lg font-semibold mb-2'>User Simulated Trades</h2>";
echo "<table class='min-w-full table-auto border'>";
echo "<thead>
        <tr class='bg-blue-200'>
          <th class='px-4 py-2 border'>Type</th>
          <th class='px-4 py-2 border'>Stock</th>
          <th class='px-4 py-2 border'>Broker</th>
          <th class='px-4 py-2 border'>Price</th>
          <th class='px-4 py-2 border'>Time</th>
        </tr>
      </thead><tbody>";

foreach (array_reverse($userTrades) as $trade) {
  echo "<tr class='text-center'>";
  echo "<td class='border px-4 py-2'>{$trade['type']}</td>";
  echo "<td class='border px-4 py-2'>{$trade['stock']}</td>";
  echo "<td class='border px-4 py-2'>{$trade['broker']}</td>";
  echo "<td class='border px-4 py-2'>\$" . number_format($trade['price'], 2) . "</td>";
  echo "<td class='border px-4 py-2'>{$trade['time']}</td>";
  echo "</tr>";
}
echo "</tbody></table>";
?>
