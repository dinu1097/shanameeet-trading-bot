...
def main_loop():
    while True:
        ...
        for symbol in stocks:
            ...
            new_opps = detect_arbitrage(symbol, atfx, th)
            arbitrage_history.extend(new_opps)

        # Print only arbitrage opportunities with history
        if arbitrage_history:
            print(f"\n⏱ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Arbitrage Opportunities (Updated Live):\n")
            print(tabulate(
                [[
                    t['Time'], t['Symbol'], t['Sell Broker'], t['Sell Price'],
                    t['Buy Broker'], t['Buy Price'], t['Profit %'], t['Type']
                ] for t in arbitrage_history],
                headers=["Time", "Symbol", "Sell Broker", "Sell Price", "Buy Broker", "Buy Price", "Profit %", "Type"],
                tablefmt="fancy_grid"
            ))
        else:
            print("\n❌ No arbitrage found yet.")
